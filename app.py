...
def calculate_risk_level(rsi: float, bb_pctb: float, vol_ratio: float) -> dict:
    """
    Calculate risk level based on error analysis reports.
    High error rate conditions from 6try and 7try_1h analysis:
    - RSI < 0.35 or RSI > 0.66: ~63-64% error rate
    - BB_PctB < 0.05 or BB_PctB > 0.95: ~68% error rate  
    - Vol_Ratio > 2.0: ~57-60% error rate
    """
    risk_reasons = []
    
    # Normalize RSI to 0-1 scale if it's in 0-100 scale
    rsi_normalized = rsi / 100.0 if rsi > 1 else rsi
    
    # Check RSI extremes (high error zones)
    if rsi_normalized < 0.35:
        risk_reasons.append(f"RSI çok düşük ({rsi:.1f}%) - Hata oranı ~64%")
    elif rsi_normalized > 0.66:
        risk_reasons.append(f"RSI çok yüksek ({rsi:.1f}%) - Hata oranı ~63%")
    
    # Check Bollinger Band position extremes
    if bb_pctb is not None:
        if bb_pctb < 0.05:
            risk_reasons.append(f"BB alt sınırında ({bb_pctb:.2f}) - Hata oranı ~68%")
        elif bb_pctb > 0.95:
            risk_reasons.append(f"BB üst sınırında ({bb_pctb:.2f}) - Hata oranı ~68%")
    
    # Check abnormal volume
    if vol_ratio is not None and vol_ratio > 2.0:
        risk_reasons.append(f"Anormal hacim ({vol_ratio:.1f}x) - Hata oranı ~57%")
...
async def scan_single_pair(pair: str, timeframe: str, model: MultiBranchModel) -> dict:
    ...
    try:
        ...
        # Get values for risk assessment
        rsi_value = float(last_candle['RSI']) if 'RSI' in last_candle else 50.0
        bb_pctb = float(last_candle['BB_PctB']) if 'BB_PctB' in last_candle else 0.5
        vol_ratio = float(last_candle['Vol_Ratio']) if 'Vol_Ratio' in last_candle else 1.0
        
        # Calculate risk level based on error analysis
        risk_info = calculate_risk_level(rsi_value, bb_pctb, vol_ratio)
...
async def run_market_scan(timeframe: str, continuous: bool = False):
    """Run a full market scan for all Binance Futures pairs."""
    global scan_state
    
    scan_state["continuous_mode"] = continuous
    ...
    while True:  # Loop for continuous mode
        ...
        cycle_info = f" (Cycle #{scan_state['scan_cycle']})" if continuous else ""
        print(f"🚀 Starting scan for {len(pairs)} pairs on {timeframe} timeframe{cycle_info}")
        ...
        if not continuous or not scan_state["is_scanning"]:
            scan_state["is_scanning"] = False
        ...
        print(f"⏳ Waiting 30 seconds before next scan cycle...")
...
async def start_scan(request: ScanRequest, background_tasks: BackgroundTasks):
    ...
    # Reset scan cycle counter on new scan
    scan_state["scan_cycle"] = 0
    
    background_tasks.add_task(run_market_scan, request.timeframe, request.continuous)
    return {"status": "started", "timeframe": request.timeframe, "continuous": request.continuous}

@app.post("/api/scan/stop")
async def stop_scan():
    """Stop the current scan (including continuous mode)."""
    global scan_state
    scan_state["is_scanning"] = False
    scan_state["continuous_mode"] = False
...
async def get_scan_status():
    ...
    return {
        ...
        "scan_cycle": scan_state["scan_cycle"],
