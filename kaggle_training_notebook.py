"""
================================================================================
🚀 KAGGLE CRYPTO MODEL TRAINER - 20 COIN × 2 TIMEFRAME = 40 MODEL
================================================================================
Bu script Kaggle notebook'a yapıştırılarak kullanılır.
Her coin için 15m ve 1h timeframe'lerinde model eğitir.
Checkpoint sistemi sayesinde notebook kapansa bile kaldığı yerden devam eder.

Kullanım:
1. Bu kodu Kaggle notebook'a yapıştırın
2. GPU accelerator'ı açın (Settings > Accelerator > GPU)
3. Çalıştırın ve bekleyin
================================================================================
"""
...
# Optuna kaldırıldı - Sabit parametreler kullanılıyor
...
# TA-Lib kurulumu (Kaggle'da özel kurulum gerekebilir)
try:
    import talib
except ImportError:
...
TIMEFRAME_CONFIGS = {
    "15m": {
        ...
        "lstm_window": 96,
        "tr_window": 96,
        "mask_threshold": 0.15  # %0.15 minimum hareket
    },
    "1h": {
        "timeframe": "1h",
        "months_back": 180,  # 15 yıl (mevcut veri kadar)
        "cnn_window": 12,
        "lstm_window": 120,
        "tr_window": 120,
        "mask_threshold": 0.11  # %0.11 minimum hareket
    }
}

# ÖNCEKİ EĞİTİMDEN EN İYİ PARAMETRELER (Optuna aramasına gerek yok!)
BEST_PARAMS = {
    "15m": {
        "embed_dim": 96,
...
# Training ayarları
FINAL_EPOCHS = 100     # Eğitim epoch sayısı

# Checkpoint dosyaları (Kaggle için ayrı okuma/yazma path'leri)
CHECKPOINT_READ_PATH = "/kaggle/input/checkpoint/training_checkpoint.json"  # Dataset olarak yüklenen
CHECKPOINT_WRITE_PATH = "/kaggle/working/training_checkpoint.json"  # Yazılabilir alan
...
def get_exchange_instance(exchange_name="binance"):
    """Borsa bağlantısını oluşturur veya cache'ten getirir."""
...
def prepare_ai_dataframe(df):
    """AI için hazır DataFrame oluşturur."""
    ...
    def forward(self, x_cnn, x_lstm, x_tr):
        ...
        if self.training and self.current_epoch >= 3:
            if torch.rand(1).item() < 0.50:
                r = torch.rand(1).item()
                if r < 0.60:
                    out_cnn = torch.zeros_like(out_cnn)
                elif r < 0.80:
                    out_lstm = torch.zeros_like(out_lstm)
                else:
                    out_tr = torch.zeros_like(out_tr)
...
def save_checkpoint(coin_idx, tf_idx, completed_models, current_state=None):
    """Checkpoint kaydet (working klasörüne)."""
    checkpoint = {
        "coin_idx": coin_idx,
        "tf_idx": tf_idx,
...
def load_checkpoint():
    """Checkpoint yükle (önce working, sonra input'tan dene)."""
    # Önce working klasöründe ara (devam eden eğitim)
    ...
    # Yoksa input'tan ara (dataset olarak yüklenen)
...
def save_model_files(coin, timeframe, model, best_params, train_ds, history):
    """Model ve ilgili dosyaları kaydet."""
    ...
    # Parameters (coin ve timeframe bilgisiyle birlikte)
...
def train_single_model(coin, tf_config, tf_name):
    """Tek bir coin-timeframe için model eğit."""
    print(f"\n{'='*60}")
    print(f"🎯 EĞİTİM: {coin} - {tf_name}")
    print(f"{'='*60}")
    ...
    # 4. Sabit parametreleri kullan (Optuna araması yapılmıyor - zaman kazanç!)
    best_params = BEST_PARAMS[tf_name]
    print(f"✨ Kullanılan parametreler: {best_params}")
    
    # 5. Final model eğitimi
    print(f"\n🏆 FINAL MODEL EĞİTİMİ ({FINAL_EPOCHS} epoch)...")
    ...
    for epoch in range(FINAL_EPOCHS):
        ...
        if avg_val_loss < best_val and epoch > 3:
            ...
            print(f"Ep {epoch+1}: Loss {avg_val_loss:.5f} | Acc %{acc:.2f} 💾")
        elif epoch % 10 == 0:
            print(f"Ep {epoch+1}: Loss {avg_val_loss:.5f} | Acc %{acc:.2f}")
...
def main():
    """Ana eğitim döngüsü."""
    print("\n" + "="*60)
    print("🚀 KAGGLE CRYPTO MODEL TRAINER")
    ...
    for coin_idx in range(start_coin_idx, len(COINS)):
        ...
        for tf_idx in range(tf_start, len(timeframes)):
            ...
            # Her model sonrası checkpoint kaydet
    ...
    summary.to_csv(os.path.join(OUTPUT_DIR, "training_summary.csv"), index=False)
    print(summary)
